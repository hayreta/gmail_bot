...
bot.hears('✅ Review Approvals', async (ctx) => {
    ...
    if (pendingIds.length === 0) {
        return ctx.replyWithMarkdown("✅ **No pending registrations to review.**", adminKeyboard);
    }
...
bot.action(/view_approval:(.+)/, async (ctx) => {
    ...
    const approvalMessage = 
        ...
        `🆔 *User ID:* \`${req.userId}\`\n` +
        `📧 *Email:* \`${req.email}\`\n` +
        `🔐 *Password Strength:* ${req.passwordStrength}\n` +
    ...
    await ctx.editMessageText(approvalMessage, {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
            [Markup.button.callback("✅ Approve", `approve:${reqId}`), Markup.button.callback("❌ Reject", `reject:${reqId}`)],
            [Markup.button.callback("⬅️ Back", "back_to_approvals")]
        ])
    });
    ctx.answerCbQuery();
});
...
bot.action(/approve:(.+)/, async (ctx) => {
    ...
    // Apply registration to user
    const user = getDB(req.userId);
    if (!user.emails) user.emails = [];
    user.emails.push(req.email);
    user.points -= 5;
    user.registered += 1;
    ...
    ctx.answerCbQuery("✅ Registration approved & user notified!", { show_alert: true });
...
bot.action(/reject:(.+)/, async (ctx) => {
    ...
    ctx.answerCbQuery("❌ Registration rejected & user notified!", { show_alert: true });
